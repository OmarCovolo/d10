...
*/
console.log("----------------------------2")
const whoIsBigger = (num,num1) => { if (num>num1){ return num;}
else if(num1>num){ return num1 }
else{ console.log("i due numeri sono uguali")}
}

console.log(whoIsBigger(14,13)); 
...
function deleteoOne(stringa, booleano) {
  lettere=stringa.split("")
  if (booleano===true){lettere.splice(0,1)
...
function onlyLetters(frase) {lettere=frase.split("")
for (let i = 0; i < lettere.length; i++) {
  
  if (typeof lettere[i] === 'number' ) { console.log(noNumbers.push(lettere[i]))
...
isThisAnEmail("pfcovolo@gmail.com")
...
function whatDayIsIt() {
  const giornis = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
  const oggi = new Date();
  const giorno = oggi.getDay();
  return giornis[giorno];
}
...
function rollTheDices(rolltimes) {
  const values = [];
...
function isTodayMyBirthday(giorno, mese) {
  ...
  if (today.getDate() === giorno && today.getMonth() === mese-1) {
    console.log(true)
